...
- [ ] **`sitemap.xml`** at `/sitemap.xml` · GENERATED FROM THE REPO by [counsel-day-complete/scripts/gen-sitemap.py](../counsel-day-complete/scripts/gen-sitemap.py) and committed; the deploy ships it as a plain file. Run `python scripts/gen-sitemap.py` after adding, retiring or noindexing a page; brand-verify fails the build if it is stale. A page is listed only if it has a `<meta name="robots">` that does not say `noindex` (default deny), `<loc>` comes from its own canonical, and `<lastmod>` from its `article:modified_time`. **Server-side generation was removed on 2026-08-15** (`regenerate-sitemap.sh`, `counsel-day-sitemap.timer/.service`, `src/jobs/sitemap.ts`): it crawled `/var/www`, published all 23 `admin-*.html` pages plus partials and noindex ad landers, dropped `engineering/`, and stamped an identical `lastmod` on every URL. Do not reintroduce it.
