...
| 6c| `purchase`              | Stripe checkout SUCCEEDS · fired on the success return (billing.html?checkout=success). `transaction_id` = Stripe session id (localStorage-guarded against refresh double-fires); value/items included when the products row supplies a display price. THE money conversion · import into Google Ads as the primary conversion for paid campaigns (docs/adwords). | billing | `transaction_id`, `value`, `currency: 'USD'`, `items` |
