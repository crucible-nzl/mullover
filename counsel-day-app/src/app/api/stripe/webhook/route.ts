...
export async function POST(req: Request) {
  ...
  try {
    switch (event.type) {
      ...
      // customer.subscription.* events · retained defensively for the
      // long-retired Consumer Annual and any historical subscription.
      // Counsel Journal (the only recurring product · $4.99 USD/mo) was
      // decommissioned 2026-08-09, so there is no subscription product left
      // to mirror; we only flip a legacy plan back to free on cancellation.
